...
export function getUserFromRequest(req: { auth?: { user?: { id: string; email: string; name: string; role: string } } }) {
  return req.auth?.user ?? null
}

export async function getServerSession(context?: GetServerSidePropsContext) {
  if (context) {
    return getNextAuthSession(context.req, context.res, authOptions)
  }
